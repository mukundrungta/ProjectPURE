...
def train_meta_learning(args, model, train_features_meta_train, train_features_meta_test, train_features,  benchmarks):
    train_dataloader_meta_train = DataLoader(train_features_meta_train, batch_size=4, shuffle=False, collate_fn=collate_fn, drop_last=True)
    train_dataloader_meta_test = DataLoader(train_features_meta_test, batch_size=4, shuffle=False, collate_fn=collate_fn, drop_last=True)
    train_dataloader = DataLoader(train_features, batch_size=args.train_batch_size, shuffle=True, collate_fn=collate_fn, drop_last=True)
    total_steps = int(len(train_dataloader) * args.num_train_epochs // args.gradient_accumulation_steps)
    warmup_steps = int(total_steps * args.warmup_ratio)
    ...
    scaler = GradScaler()
    optimizer = AdamW(model.parameters(), lr=args.learning_rate, eps=args.adam_epsilon)
    inner_opt = SGD(model.parameters(), lr=0.001)
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warmup_steps, num_training_steps=total_steps)
    print('Total steps: {}'.format(total_steps))
    print('Warmup steps: {}'.format(warmup_steps))
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (batch_meta_train, batch_meta_test) in enumerate(tqdm(zip(train_dataloader_meta_train, train_dataloader_meta_test))):
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                scheduler.step()
                model.zero_grad()
                wandb.log({'meta-loss': total_loss.item()}, step=num_steps)
...
def main():
    ...
    train_features_meta_train, train_features_meta_test = processor.read_metalearning(train_file) #get features for meta-learning
    train_features = processor.read_normal(train_file) #get features for examples not in meta-learning schema

    print('Number of meta-train features = ' +str(len(train_features_meta_train)))
    print('Number of meta-test features = ' +str(len(train_features_meta_test)))
    print('Number of normal features = ' +str(len(train_features)))
