...
class Processor:
    ...
    def tokenize(self, tokens, subj_type, obj_type, ss, se, os, oe, tag):
        """
...
class TACREDProcessorToken(Processor):
    def __init__(self, args, tokenizer):
    ...
    def read(self, file_in, file_complex):
        features = []
        with open(file_in, "r") as fh:
            data = json.load(fh)
        
        with open(file_complex, "r") as fh:
            is_complex = json.load(fh)
        ...
        for d in tqdm(data):
            ...
            input_ids, new_ss, new_os = self.tokenize(tokens, d['subj_type'], d['obj_type'], ss, se, os, oe, tag)
...
class RETACREDProcessorToken(Processor):
    def __init__(self, args, tokenizer):
        super().__init__(args, tokenizer)
