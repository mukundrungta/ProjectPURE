...
from utils_datamap import set_seed, collate_fn
from prepro_datamap import RETACREDProcessor
...
def train(args, model, train_features, benchmarks):
    ...
    for epoch in range(int(args.num_train_epochs)):
        model.zero_grad()
        training_dynamics_epoch = []
        challenge_dynamics_epoch = []
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            for index, guid in enumerate(batch[5]):
                # print(guid + " :::: " +str(logits[index].tolist()))
                # print("Label = " +str(batch[2][index].item()))
                training_dynamics_epoch.append({"guid":guid, "logits_epoch_"+str(epoch): logits[index].tolist(), "gold":batch[2][index].item()})
            ###########################################################################################################################
        ...
        ########################################## get the result for the challenge data after this epoch
        tag_challenge = "challenge_test"
        challenge_features = benchmarks[2][1]
        dataloader = DataLoader(challenge_features, batch_size=args.test_batch_size, collate_fn=collate_fn, drop_last=False)
        keys, preds = [], []
        for i_b, batch in enumerate(dataloader):
            ...
            for index, guid in enumerate(batch[5]):
                # print(guid + " :::: " +str(logits[index].tolist()))
                # print("Label = " +str(batch[2][index].item()))
                challenge_dynamics_epoch.append({"guid":guid, "logits_epoch_"+str(epoch): logits[index].tolist(), "gold":batch[2][index].item()})
            
        #################################################################################################

        # with open(args.data_dir + '/datamap_input/dynamics_epoch_' +str(epoch) +'.jsonl', 'w') as f:
        #     for item in training_dynamics_epoch:
        #         f.write("%s\n" % item)
        # with open(args.data_dir + '/datamap_input/dynamics_epoch_' +str(epoch) +'.jsonl', "w") as final:
        #     json.dump(training_dynamics_epoch, final)
        with open(os.path.join(args.data_dir, args.output_dir) + '/dynamics_epoch_' +str(epoch) +'.jsonl', 'w') as fp:
            fp.write('\n'.join(json.dumps(i) for i in training_dynamics_epoch))
        
        with open(os.path.join(args.data_dir, args.output_dir) + '/challenge_dynamics_epoch_' +str(epoch) +'.jsonl', 'w') as fp:
            fp.write('\n'.join(json.dumps(i) for i in challenge_dynamics_epoch))
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="datamap_input")
