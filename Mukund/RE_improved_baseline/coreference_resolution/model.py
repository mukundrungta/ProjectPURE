...
class REModelToken(nn.Module):
    def __init__(self, args, config):
        super().__init__()
        self.args = args
        self.encoder = AutoModel.from_pretrained(args.model_name_or_path, config=config)
        hidden_size = config.hidden_size
        self.token_emb_size = 100
        self.embedding = nn.Embedding(2, self.token_emb_size) # [EASY], [COMPLEX] two words in vocab
        self.loss_fnt = nn.CrossEntropyLoss()
        self.classifier = nn.Sequential(
            nn.Linear(2 * hidden_size + self.token_emb_size, hidden_size),
            nn.ReLU(),
            nn.Dropout(p=args.dropout_prob),
            nn.Linear(hidden_size, args.num_class)
        )
    ...
    @autocast()
    def forward(self, input_ids=None, attention_mask=None, labels=None, ss=None, os=None, tag=None):
        outputs = self.encoder(
            input_ids,
            attention_mask=attention_mask,
        )
        ...
        tag_embedding = self.embedding(lookup_tensor)
        h = torch.cat((ss_emb, os_emb, tag_embedding), dim=-1)
        logits = self.classifier(h)
        outputs = (logits,)
        if labels is not None:
            loss = self.loss_fnt(logits.float(), labels)
