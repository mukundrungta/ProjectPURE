...
def print_pred_json(eval_data, eval_examples, preds, id2label, output_file):
    ...
    example_document = []
    for ex, pred in zip(eval_examples, preds):
        ex['pred_relation'] = id2label[pred]
        example_document.append(ex)
    print(example_document[0])
    print(type(example_document[0]))
    with open(os.path.join(args.output_dir, 'datasetACE05.json'), 'w') as f:
      f.write('\n'.join(json.dumps(doc, default=myconverter) for doc in example_document))
      # for doc in example_document:
        # f.write(json.dumps(doc, default=myconverter))
    # with open(os.path.join(args.output_dir, 'dataset.json'), 'w') as f:
    #     f.write('\n'.join(json.dumps(doc) for doc in eval_examples))
